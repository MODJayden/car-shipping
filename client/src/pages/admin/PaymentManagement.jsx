...
import { getAllShippings, updateShipping } from "../../store/shipping";
...
const AdminShippingManagement = () => {
  ...
  const openUpdateDialog = (shipping) => {
  ...
  const handleUpdateShipping = () => {
    ...
    dispatch(
      updateShipping({ id: selectedShipping._id, formData: updateForm })
    ).then((res) => {
      if (res.payload.success) {
        toast.success("Shipping status updated successfully");
        dispatch(getAllShippings());
        setIsUpdateDialogOpen(false);
        setUpdateForm({ status: "", trackingNumber: "" });
      } else {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Monitor and manage all vehicle shipments and deliveries
          </p>
        </div>
